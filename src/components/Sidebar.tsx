...
import { LayoutDashboard, LineChart, PieChart, Star, Settings, LogOut, TrendingUp, ClipboardList } from 'lucide-react';
  ...
  return (
    <div className="w-64 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col h-screen sticky top-0">
      <div className="p-6 flex items-center space-x-3">
        ...
        <h1 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight">StockAI</h1>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`w-5 h-5 ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 group-hover:text-gray-600'}`} />
              <span>{item.name}</span>
              {isActive && (
                <motion.div 
                  layoutId="activeNav"
